...
public class OpenGLFlag {
    ...
    private void init() {
        ByteBuffer bb1 = ByteBuffer.allocateDirect(AppConstants.FLAG_RESOLUTION * 8 * Float.SIZE);
        bb1.order(ByteOrder.nativeOrder());
        fbCoords = bb1.asFloatBuffer();

        ByteBuffer bb2 = ByteBuffer.allocateDirect(AppConstants.FLAG_RESOLUTION * 8 * Float.SIZE);
        bb2.order(ByteOrder.nativeOrder());
        fbTexture = bb2.asFloatBuffer();

        coords = new float[AppConstants.FLAG_RESOLUTION * 12];
        int i = 0;

        for (int x = 0; x < AppConstants.FLAG_RESOLUTION; x++) {

            float z1 = centerZ + (float) (Math.sin(x * 2 * Math.PI / (AppConstants.FLAG_RESOLUTION - 1)) / AppConstants.FLAG_WAVE_LENGTH);
            float z2 = centerZ + (float) (Math.sin((x + 1) * 2 * Math.PI / (AppConstants.FLAG_RESOLUTION - 1)) / AppConstants.FLAG_WAVE_LENGTH);

            coords[i] = centerX - width / 2 + x * width / AppConstants.FLAG_RESOLUTION;
            i++;
            ...
            coords[i] = z1;
            i++;
            coords[i] = centerX - width / 2 + (x + 1) * width / AppConstants.FLAG_RESOLUTION;
            i++;
            coords[i] = centerY - height / 2;
            i++;
            coords[i] = z2;
            ...
            float dx1 = (float) x / AppConstants.FLAG_RESOLUTION;
            float dx2 = (float) (x + 1) / AppConstants.FLAG_RESOLUTION;
    ...
    public void draw(GL10 gl, boolean paused) {
        ...
        if (!paused && skip >= AppConstants.FLAG_SKIP_FRAMES) {
            float f1, f2, f3, f4;
